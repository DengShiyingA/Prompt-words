        ...
        <div style={{ padding: '28px 32px' }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 16 }}>
            <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap' }}>
              {item.tags.map(t => (
                <span key={t} style={{ fontSize: 12, background: '#f5f5f7', color: '#6e6e73', borderRadius: 980, padding: '3px 10px' }}>{t}</span>
              ))}
              <span style={{ fontSize: 12, background: 'rgba(94,92,230,0.1)', color: '#5e5ce6', borderRadius: 980, padding: '3px 10px' }}>{item.model}</span>
            </div>
          ...
          <div style={{ display: 'flex', gap: 10, alignItems: 'center' }}>
            ...
            <button
              ...
              style={{ display: 'flex', alignItems: 'center', gap: 5, background: item.liked ? '#ff375f' : '#f5f5f7', color: item.liked ? '#fff' : '#1d1d1f', border: 'none', borderRadius: 980, padding: '6px 14px', fontSize: 12, cursor: 'pointer', transition: 'all 0.2s' }}
            >
              <Heart size={12} fill={item.liked ? '#fff' : 'none'} /> {item.likes}
            </button>
            <button
              onClick={() => onFavorite(item.id)}
              style={{ display: 'flex', alignItems: 'center', gap: 5, background: item.favorited ? '#ff9500' : '#f5f5f7', color: item.favorited ? '#fff' : '#1d1d1f', border: 'none', borderRadius: 980, padding: '6px 14px', fontSize: 12, cursor: 'pointer', transition: 'all 0.2s' }}
            >
